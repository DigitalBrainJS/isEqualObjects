...
export default function isEqualObjects(obj, targetObj) {
    const isEqual = (value, targetValue) => {
        ...
        if (value !== value && targetValue !== targetValue) return true;

        if(typeof value==='object' && typeof targetValue==="object" && value && targetValue) {
            if (isArray(value)) {
                if (isArray(targetValue)) {
                    const {length} = value;
                    if (length !== targetValue.length) return false;
                    for (let i = 0; i < length; i++) {
                        if (!isEqual(value[i], targetValue[i])) return false;
                    }
                    return true;
                }
                ...
                const {length} = keys;
                if (length !== targetKeys.length) return false;
                for (let i = 0; i < length; i++) {
                    const key = keys[i];
                    if (!hasOwnProperty.call(targetValue, key)) return false;
